#!/usr/bin/env python3
"""NotTheNet Windows - Fake Internet Simulator for Windows.

Windows-native entry point (no iptables, no privilege dropping).
Two runtime modes:
  ...
  Headless mode  (--headless flag or NTN_HEADLESS=1 env var)
    ...
    and blocks until SIGTERM/SIGINT. Suitable for automation and scripts.
...
Architecture layers (Clean Architecture):
  ...
  infrastructure/  Concrete adapters: DI container, env config, health HTTP
...
try:
    ...
    print(
        f"[!] Current working directory is stale (deleted) — chdir() to {_SCRIPT_DIR}",
        file=sys.stderr,
    )
    os.chdir(_SCRIPT_DIR)

from gui.widgets import APP_VERSION  # noqa: F401,E402


def _headless_main() -> None:
    """Run in headless / automation mode: no GUI, health endpoint active."""
    import logging
    ...
    container = Container.build(config_path=os.environ.get("NTN_CONFIG_PATH"))
    ...
    logger.info("NotTheNet Windows %s starting in headless mode", APP_VERSION)
...
def main() -> None:
    headless = "--headless" in sys.argv or os.environ.get("NTN_HEADLESS", "0").strip().lower() in (
        "1",
        "true",
        "yes",
    )
