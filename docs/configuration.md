...
| `kill_switch_domains` | array | `[]` | Domains that always return NXDOMAIN, regardless of entropy. Subdomains also match. Use this to keep malware executing past its own kill switch. **How kill switches work:** many samples (WannaCry being the most famous) call `InternetOpenUrl()` against a hardcoded domain at startup. If the HTTP connection succeeds — any response at all — the malware treats this as a sign it's inside a researcher's sandbox and exits. If the connection fails, it assumes it's on the real internet and continues. By returning NXDOMAIN for the kill-switch domain, DNS resolution fails immediately, `InternetOpenUrl()` never gets a TCP connection, and the kill switch cannot fire. **Note:** this only works for samples whose kill-switch domains you know in advance. For unknown samples, find the domain via static analysis or threat intel (VirusTotal, MalwareBazaar, abuse.ch) before detonating. |
